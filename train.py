...
from datasets.vidcoco import build_vidcoco
...
def main(cfg=cfg):
    ...
    dataset_train = build_vidcoco(image_set='train', cfg=cfg)

    # dataset_train =build_vidmulti(image_set='train', cfg=cfg, split_name='VID_train_3frames')

    dataset_val = build_vidcoco(image_set='val', cfg=cfg)
    ...
    for epoch in range(start_epoch, cfg.TRAIN.epochs):
        ...
        if cfg.TRAIN.output_dir and utils.is_main_process():
            ...
            if coco_evaluator is not None:
                ...
                if "boxes" in coco_evaluator.coco_eval:
                    filenames = ['latest.pth']
                    if epoch % 50 == 0:
                        filenames.append(f'{epoch:03}.pth')
                    for name in filenames:
                        torch.save(coco_evaluator.coco_eval["boxes"].eval,
